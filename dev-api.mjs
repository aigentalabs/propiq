...
// --- SHARED HELPERS (mirrored from api/valuation.ts) ---
...
// Mocking Vercel's req/res for the local server
const handler = async (req, res) => {
    ...
    req.on('end', async () => {
        try {
            const { property, images } = JSON.parse(body);
            const apiKey = process.env.GEMINI_API_KEY;

            if (!apiKey) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ message: 'GEMINI_API_KEY not configured' }));
                return;
            }

            // --- RESPONSE SCHEMA (mirrored from api/valuation.ts) ---
            ...
            // --- BUILD PROMPT (mirrored from api/valuation.ts) ---
...
const areaEstimationHandler = async (req, res) => {
    ...
    req.on('end', async () => {
        try {
            const { image, roomLabel } = JSON.parse(body);
            const apiKey = process.env.GEMINI_API_KEY;
            if (!apiKey) { res.writeHead(500); res.end(JSON.stringify({ error: 'No API key' })); return; }
...
const server = http.createServer((req, res) => {
    ...
    if (req.url === '/api/valuation') {
        handler(req, res);
    } else if (req.url === '/api/estimateArea') {
        areaEstimationHandler(req, res);
    } else {
        res.writeHead(404);
        res.end();
    }
...
server.listen(3002, () => {
    ...
    console.log('  → /api/valuation');
    console.log('  → /api/estimateArea');
});
